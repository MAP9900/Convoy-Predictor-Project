...
class Gradient_Boosting_Optimization:
    ...
    def __init__(self, model = None, parameter_grid = None, cv_folds:int = 5,feature_names:list = None, positive_label = 1, 
                 ...
                 random_state: int = 1945, save_plots: bool = False):
        """
        Class Initializer
            ...
            save_plots: When True, figures generated by evaluate() are also saved to the Plots folder.
            
            Note: Class is intended for gradient boosting models that do not require additional scaling.
        """
        if model is not None and not hasattr(model, "fit"): #Early debugging step, legacy of orginal class
        ...
        self.plot_output_dir = "/Users/matthewplambeck/Desktop/Convoy Predictor/Plots" #My file path to plotting folder
        if self.save_plots:
    ...
    def evaluate(self, show_plots: bool = False, print_results: bool = True, save_plots: bool = None):
        """
        ...
        save_plots: optional override to enable/disable saving plots for this call only.
        Legacy of ML_Class_1.py with some changes (Removed Pipeline aspects)
        """
        original_save_setting = self.save_plots
        ...
        try:
            ...
            if show_plots:
                ...
                print(f"{model_name} Confusion Matrix Plot:")
            ...
            self.save_plots = original_save_setting
    ...
    def plot_roc_curve(self, fpr, tpr, ROC_AUC):
        ...
        if self.save_plots:
            os.makedirs(self.plot_output_dir, exist_ok=True)
            model_name = type(self.best_model).__name__ if self.best_model is not None else "model"
            file_path = os.path.join(self.plot_output_dir, f"{model_name}_roc_curve.png")
    ...
    def plot_confusion_matrix(self, cm, class_labels=None):
        ...
        if self.save_plots:
            os.makedirs(self.plot_output_dir, exist_ok=True)
            model_name = type(self.best_model).__name__ if self.best_model is not None else "model"
            file_path = os.path.join(self.plot_output_dir, f"{model_name}_confusion_matrix.png")
            fig.savefig(file_path, bbox_inches='tight')
        plt.show()
        plt.close(fig)
    ...
    def plot_feature_importance(self):
        ...
        if self.feature_names is not None:
            feature_labels = list(self.feature_names)
        if not feature_labels:
            feature_labels = [f"Feature_{i}" for i in range(len(importances))]
        elif len(feature_labels) != len(importances):
            if len(feature_labels) > len(importances):
                feature_labels = feature_labels[:len(importances)]
            else:
                feature_labels = feature_labels + [f"Feature_{i}" for i in range(len(feature_labels), len(importances))]

        feature_importance_df = pd.DataFrame({'Feature': feature_labels,'Importance': importances}).sort_values\
            (by='Importance', ascending=False)
        feature_importance_df['Feature'] = pd.Categorical(
            feature_importance_df['Feature'],
            categories=feature_importance_df['Feature'],
            ordered=True
        )
        fig = plt.figure(figsize=(8,4), facecolor='lightgrey')
        ...
        if self.save_plots:
            os.makedirs(self.plot_output_dir, exist_ok=True)
            model_name = type(self.best_model).__name__ if self.best_model is not None else "model"
            file_path = os.path.join(self.plot_output_dir, f"{model_name}_feature_importance.png")
            fig.savefig(file_path, bbox_inches='tight')
        plt.show()
        plt.close(fig)
        return
